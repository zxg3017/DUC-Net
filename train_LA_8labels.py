...
def train_main():
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ## unsupervised loss
            PUC_loss = losses.dice_loss(outputs_u_soft[:labeled_bs, ...], U_data_pseudo[:].float())  # 这个是计算真实分割结果和预测结果的空间距离误差
            # PUC_loss = ce_loss(outputs_u_soft[:2, ...], U_data_pseudo[:].float())  # 这个是计算真实分割结果和预测结果的空间距离误差

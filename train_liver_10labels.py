...
from test_util import test_all_liver_case

from networks.vnet_sdf import VNet
#from networks_v1.discriminator import FC3DDiscriminator

from dataloaders import utils
from dataloaders.la_heart import LAHeart,Liver_data, RandomCrop, CenterCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler,  CreateOnehotLabel
...
parser.add_argument('--exp', type=str,
                    default='Liver/Liver_10labels', help='model_name')
...
parser.add_argument('--labelnum', type=int,  default=10, help='random seed')
...
def train_main():
    ...
    db_train = Liver_data(base_dir=train_data_path,
...
# 输入: batch_size x 1 x patch_size, 标签: batch_size x class x patch_size(one_hot形式)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_tanh, outputs = model(inputs)  # outputs_tanh -> (-1, 1), outputs无限制
            ...
            # supervised loss
            supervised_loss = ce_loss(outputs[:labeled_bs, 1, ...], X_label[:].float())  # 这个是计算真实分割结果和预测结果的交叉熵误差
            ## unsupervised loss
            PUC_loss = losses.dice_loss(outputs_u_soft[:labeled_bs, ...],
                                        U_data_pseudo[:].float())  # 这个是计算真实分割结果和预测结果的空间距离误差
...
def test():
    ...
    snapshot_path = '/mnt/ai2019/zxg_FZU/semi-supervised/model_liver/Liver/Liver_10labels/'

    num_classes = 2

    test_save_path = os.path.join(snapshot_path, "test_LA/")
    ...
    avg_metric = test_all_liver_case(net, filename_list, num_classes=num_classes,

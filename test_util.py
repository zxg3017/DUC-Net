...
def test_all_liver_case(net, filename_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,
    ...
    for index in range(len(filename_list)):
        ...
        ct = sitk.ReadImage(filename_list[index][0], sitk.sitkInt16)
        seg = sitk.ReadImage(filename_list[index][1], sitk.sitkUInt8)
    ...
    avg_metric = total_metric / len(filename_list)
    metrics_for_csv.append(['AVG', avg_metric[0], avg_metric[1], avg_metric[2], avg_metric[3]])
    results_file = open(test_save_path + 'result.csv', 'w', newline='')
    csv_writer = csv.writer(results_file, dialect='excel')
    for row in metrics_for_csv:
